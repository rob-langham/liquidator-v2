...
import { handleAssetAdded, handleVaultBuilt, handleVaultDestroyed, handleVaultGiven, handleVaultPoured, handleVaultRolled, handleVaultTweaked } from "../src/contango-cauldron"
import { createAssetAddedEvent, createVaultBuiltEvent, createVaultDestroyedEvent, createVaultGivenEvent, createVaultPouredEvent, createVaultRolledEvent, createVaultTweakedEvent } from "./contango-cauldron-utils"
...
describe("Vault events", () => {
  ...
  describe("Vault poured", () => {
    test("should set initial balance", () => {
      ...
      thenVaultBalanceShouldMatch(vaultId, BigInt.fromI32(100), BigInt.fromI32(200))
      assert.entityCount("Vault", 1);
    });
    ...
    test("should set delat on ink", () => {
      ...
      assert.entityCount("Vault", 1);
      thenVaultBalanceShouldMatch(vaultId, BigInt.fromI32(50), BigInt.fromI32(200))
    });
    ...
    test("should set delta on art", () => {
      ...
      assert.entityCount("Vault", 1);
      thenVaultBalanceShouldMatch(vaultId, BigInt.fromI32(100), BigInt.fromI32(50))
    });
    ...
    test("should set delta on both", () => {
      ...
      assert.entityCount("Vault", 1);
      thenVaultBalanceShouldMatch(vaultId, BigInt.fromI32(50), BigInt.fromI32(50))
    });
  ...
  describe("VaultRolled", () => {
    ...
    test("should update art", () => {
      ...
      assert.entityCount("Vault", 1);
      thenVaultBalanceShouldMatch(vaultId, BigInt.fromI32(0), BigInt.fromI32(200))
    })
  });
  ...
  describe("VaultGiven", () => {
    test("should update owner", () => {
      givenVaultBuiltEvent()
      givenVaultGivenEvent(vaultId, Address.fromString("0x0003000600000800000000000000000000000002"))

      assert.entityCount("Vault", 1);
      thenVaultOwnerShouldMatch(vaultId, Address.fromString("0x0003000600000800000000000000000000000002"))
    });
...
function givenVaultPouredEvent(ink: BigInt, art: BigInt): void {
  handleVaultPoured(createVaultPouredEvent(
    Bytes.fromHexString(vaultIdHex(vaultId)),
    seriesId,
    ilkId,
    ink,
...
function givenVaultTweakedEvent(vaultId: Bytes, seriesId: Bytes, ilkId: Bytes): void {
  handleVaultTweaked(createVaultTweakedEvent(
    Bytes.fromHexString(vaultIdHex(vaultId)),
    seriesId,
    ilkId,
  ));
...
function givenVaultRolledEvent(vaultId: Bytes, seriesId: Bytes, art: BigInt): void {
  handleVaultRolled(createVaultRolledEvent(
    Bytes.fromHexString(vaultIdHex(vaultId)),
    seriesId,
    art,
  ));
...
function givenVaultGivenEvent(vaultId: Bytes, owner: Address): void {
  handleVaultGiven(createVaultGivenEvent(
    Bytes.fromHexString(vaultIdHex(vaultId)),
    owner,
  ));
}
...
function whenVaultDestroyedEvent(vaultId: Bytes): void {
  handleVaultDestroyed(createVaultDestroyedEvent(
    Bytes.fromHexString(vaultIdHex(vaultId)),
  ));
}
...
function thenVaultAssestShouldMatch(vaultId: Bytes, seriesId: Bytes, ilkId: Bytes): void {
  assert.fieldEquals("Vault", vaultIdHex(vaultId), "seriesId", seriesId.toHex())
  assert.fieldEquals("Vault", vaultIdHex(vaultId), "ilkId", ilkId.toHex())
}

function thenVaultBalanceShouldMatch(vaultId: Bytes, ink: BigInt | null, art: BigInt | null): void {
  if (ink !== null)
...
function vaultIdHex(id: Bytes | null): string {
  return "0x" + (id ? id : vaultId).toHex().replace("0x", "").padStart(24, '0')
}